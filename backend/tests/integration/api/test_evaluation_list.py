...
def test_filter_by_category_fashion(client):
    """Test category=fashion returns only fashion evaluations."""
    with (
        patch("app.core.dependencies.verify_firebase_token") as mock_verify,
        patch("app.core.dependencies.db") as mock_db,
        patch("app.core.dependencies.user_queries") as mock_user_queries,
        ...
        mock_svc_conn = _setup_search_mocks(
            mock_verify, mock_db, mock_user_queries, mock_svc_db,
            fetch_return=[EVAL_ROW_1, EVAL_ROW_3], fetchval_return=2,
        )

        response = client.get(
            "/api/v1/evaluations?category=fashion",
            headers=AUTH_HEADERS,
        )
        ...
        assert data["total"] == 2
        assert len(data["items"]) == 2
        assert all(item["template"] == "fashion" for item in data["items"])

        # Verify SQL includes template filter
        fetch_call = mock_svc_conn.fetch.call_args
        query_sql = fetch_call.args[0]
        assert "e.template = $" in query_sql


def test_filter_by_category_non_fashion(client):
    """Test category=non_fashion returns only non-fashion evaluations."""
    with (
        patch("app.core.dependencies.verify_firebase_token") as mock_verify,
        patch("app.core.dependencies.db") as mock_db,
        patch("app.core.dependencies.user_queries") as mock_user_queries,
        ...
        _setup_search_mocks(
            mock_verify, mock_db, mock_user_queries, mock_svc_db,
            fetch_return=[EVAL_ROW_2], fetchval_return=1,
        )

        response = client.get(
            "/api/v1/evaluations?category=non_fashion",
            headers=AUTH_HEADERS,
        )
        ...
        assert data["total"] == 1
        assert len(data["items"]) == 1
        assert data["items"][0]["template"] == "non_fashion"


def test_filter_no_category_returns_all(client):
    """Test omitting category param returns all evaluations (existing behavior)."""
    with (
        patch("app.core.dependencies.verify_firebase_token") as mock_verify,
        patch("app.core.dependencies.db") as mock_db,
        patch("app.core.dependencies.user_queries") as mock_user_queries,
        ...
        # Verify SQL does NOT include template filter
        fetch_call = mock_svc_conn.fetch.call_args
        query_sql = fetch_call.args[0]
        assert "e.template = $" not in query_sql


def test_filter_category_combined_with_search(client):
    """Test category + search filters combine with AND logic."""
    with (
        patch("app.core.dependencies.verify_firebase_token") as mock_verify,
        patch("app.core.dependencies.db") as mock_db,
        patch("app.core.dependencies.user_queries") as mock_user_queries,
        ...
        response = client.get(
            "/api/v1/evaluations?search=Nike&category=fashion",
            headers=AUTH_HEADERS,
        )
        ...
        assert data["total"] == 1
        assert data["items"][0]["brand_name"] == "Nike Indonesia"
        assert data["items"][0]["template"] == "fashion"

        # Verify SQL has both ILIKE and template conditions
        fetch_call = mock_svc_conn.fetch.call_args
        query_sql = fetch_call.args[0]
        query_params = fetch_call.args[1:]
        assert "ILIKE" in query_sql
        assert "e.template = $" in query_sql
        # Param order: $1=search, $2=category, $3=limit, $4=offset
        assert query_params[0] == "Nike"
        assert query_params[1] == "fashion"
        assert len(query_params) == 4


def test_filter_category_combined_with_date_range(client):
    """Test category + date range filters combine with AND logic."""
    with (
        patch("app.core.dependencies.verify_firebase_token") as mock_verify,
        patch("app.core.dependencies.db") as mock_db,
        patch("app.core.dependencies.user_queries") as mock_user_queries,
        ...
        response = client.get(
            "/api/v1/evaluations?date_from=2026-02-01&date_to=2026-02-28&category=fashion",
            headers=AUTH_HEADERS,
        )
        ...
        # Verify SQL has date and template conditions
        fetch_call = mock_svc_conn.fetch.call_args
        query_sql = fetch_call.args[0]
        assert "created_at >=" in query_sql
        assert "interval '1 day'" in query_sql
...
def test_filter_category_combined_with_all_filters(client):
    """Test search + date_from + date_to + category all apply as AND."""
    with (
        patch("app.core.dependencies.verify_firebase_token") as mock_verify,
        patch("app.core.dependencies.db") as mock_db,
        patch("app.core.dependencies.user_queries") as mock_user_queries,
        ...
        response = client.get(
            "/api/v1/evaluations?search=Nike&date_from=2026-02-01&date_to=2026-02-28&category=fashion",
            headers=AUTH_HEADERS,
        )
        ...
        # Verify all four conditions in SQL
        fetch_call = mock_svc_conn.fetch.call_args
        query_sql = fetch_call.args[0]
        query_params = fetch_call.args[1:]
        assert "ILIKE" in query_sql
        ...
        assert "interval '1 day'" in query_sql
        assert "e.template = $" in query_sql
        # Param order: $1=search, $2=date_from, $3=date_to, $4=category, $5=limit, $6=offset
        assert query_params[0] == "Nike"
        assert query_params[3] == "fashion"
        assert len(query_params) == 6


def test_filter_category_with_pagination(client):
    """Test category-filtered results have correct total/pages."""
    with (
        patch("app.core.dependencies.verify_firebase_token") as mock_verify,
        patch("app.core.dependencies.db") as mock_db,
        patch("app.core.dependencies.user_queries") as mock_user_queries,
        ...
        _setup_search_mocks(
            mock_verify, mock_db, mock_user_queries, mock_svc_db,
            fetch_return=[EVAL_ROW_1], fetchval_return=5,
        )

        response = client.get(
            "/api/v1/evaluations?category=fashion&limit=2&page=1",
            headers=AUTH_HEADERS,
        )
...
def test_filter_invalid_category_returns_422(client):
    """Test invalid category value returns 422 (FastAPI Literal validation)."""
    with (
        patch("app.core.dependencies.verify_firebase_token") as mock_verify,
        patch("app.core.dependencies.db") as mock_db,
        patch("app.core.dependencies.user_queries") as mock_user_queries,
        ...
        response = client.get(
            "/api/v1/evaluations?category=invalid",
            headers=AUTH_HEADERS,
        )
