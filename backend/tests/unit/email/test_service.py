...
class TestSmtpSendSync:
    ...
    def test_calls_starttls_login_send(self, mock_smtp: tuple) -> None:
        ...
        mock_instance.send_message.assert_called_once_with(msg)
...
class TestSendEvaluationEmail:
    ...
    async def test_generates_default_subject(
        ...
        # Check the render function received chart_cid, header_cid, footer_cid
        call_kwargs = mock_render_fn.call_args[1]
        assert "chart_cid" in call_kwargs
        assert "header_cid" in call_kwargs
        assert "footer_cid" in call_kwargs
